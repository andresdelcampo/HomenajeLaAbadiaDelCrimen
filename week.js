...
(() => {
  ...
  const events = {
    ...
    '2-2': { text: t('El Abad prohíbe la biblioteca. En el scriptorium, Malaquías puede ofrecer la visita de Berengario. Severino puede presentarse desde este día, en tercia o nona, si encuentra a Guillermo y ninguna otra conversación se interpone.', 'The Abbot forbids entry to the library. In the scriptorium, Malachi can offer Berengar’s tour. From this day, Severinus can introduce himself at terce or none if he meets William and no other conversation interferes.'), action: t('Escucha la orden del Abad y visita el scriptorium. Acércate a los monjes para iniciar sus conversaciones; Severino puede presentarse tanto en tercia como en nona.', 'Hear the Abbot’s order and visit the scriptorium. Approach the monks to trigger conversations; Severinus’s introduction has no guaranteed time.'), q: [[2,22],[3,52],[4,53],[4,54],[5,55]] },
    '2-4': { text: t('Berengario vigila el manuscrito de Venancio. Si Guillermo lo toma, la advertencia puede convertirse en una denuncia al Abad.', 'Berengar watches Venantius’s manuscript. If William takes it, the warning can become a report to the Abbot.'), action: t('Planifica cómo conservar el pergamino. La amenaza se activa si Guillermo lo lleva, pasa cierto tiempo y cambia de pantalla; puede ocurrir fuera de nona.', 'Plan how to keep the scroll. The threat depends on carrying it, elapsed time and changing screens; it is not an automatic none conversation.'), q: [[4,4]] },
    '3-0': { text: t('Berengario se encapucha, sale de la celda común, busca el libro y lo lleva a la celda de Severino. Muere al llegar con él; entonces avanza la hora. Esta es la noche anterior a la presentación de Jorge.', 'Berengar puts on a hood, leaves the shared cell, seeks the book and carries it to Severinus’s cell. He dies on arriving with it; the hour then advances. This is the night before Jorge’s introduction.'), action: t('Puedes observar el recorrido, aunque seguir al encapuchado no basta para acceder a la biblioteca. Conserva el pergamino: al amanecer se oculta tras el espejo si Guillermo no lo lleva.', 'You can watch the journey, but following the hooded monk does not solve access to the library. Keep the scroll: at dawn it is hidden behind the mirror if William is not carrying it.'), q: [] },
    ...
    '3-3': { text: t('Jorge regresa a la celda común y permanece allí hasta que desaparece de la escena en nona.', 'Jorge returns to the shared cell and becomes inactive on arrival. At none the scheduled action removes him from the scene.') },
    ...
    '5-0': { text: t('Durante la noche aparecen la llave I en el altar y las gafas en la habitación iluminada del laberinto. La llave desaparece en prima si Guillermo no la lleva.', 'The night action places key I on the altar and the spectacles in the illuminated maze room. The key disappears at prime unless William carries it.'), action: t('Recoge la llave del altar durante esta noche. Permite acceder a la celda del Abad; procura que no te sorprenda al volver.', 'Collect the altar key during this night. It opens the Abbot’s cell; account for his watchfulness on your return.') },
  ...
  const objectAdvice = {
    ...
    '3-1': t('Desde esta prima hay una lámpara en la cocina. Haz que Adso la recoja antes de explorar las zonas oscuras de la biblioteca.', 'From this prime, a lamp is available in the kitchen. Have Adso collect it before exploring the library’s dark rooms.'),
    '5-0': t('Guillermo debe conservar la llave I hasta prima: recogerla y dejarla no evita su desaparición. Las gafas que reaparecen esta noche no tienen otra retirada al amanecer.', 'William must keep key I until prime: picking it up and dropping it does not prevent its disappearance. The spectacles that reappear tonight have no further dawn removal.'),
    '5-1': t('La llave I desaparece del altar si Guillermo no la lleva al llegar prima. Las gafas siguen disponibles en la habitación iluminada si aún no las has recogido.', 'Key I disappears from the altar unless William carries it at prime. The spectacles remain available in the illuminated room if you have not collected them yet.'),
    '6-0': t('Reparte el equipo: Guillermo lleva las llaves I y II, los guantes, las gafas y el pergamino; Adso lleva la llave III y la lámpara.', 'Divide the equipment: William carries keys I and II, the gloves, spectacles and scroll; Adso carries key III and the lamp.')
  };
  const lampReminder = t('Si has usado la lámpara, en prima se retira de Adso y vuelve a la cocina, aunque aún le quedara aceite. Haz que la recoja de nuevo antes de volver a la oscuridad.', 'If you have used the lamp, at prime it is removed from Adso and returns to the kitchen, even with oil remaining. Have him collect it again before returning to the dark.');
  const places = {
  ...
  function cast(d, h) {
    ...
    if (d === 7 && h === 2) rows.forEach(r => { r.to = null; r.from = null; r.note = t('La investigación ha terminado; los personajes ya no emprenden nuevos recorridos.','Investigation deadline: no new gameplay route.'); });
    return rows;
  }
  ...
  function render() {
    ...
    const objectNotes = [objectAdvice[`${day}-${hour}`], day>=4&&hour===1 ? lampReminder : ''].filter(Boolean);
    put('[data-week-advice]',[event?.action||advice[hour], ...objectNotes].join(' '));
    find('[data-week-labyrinth]').hidden = !((day===6||day===7)&&hour===0);

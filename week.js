...
(() => {
  ...
  const events = {
    ...
    '5-0': { text: t('Durante la noche aparecen la llave I en el altar y las gafas en la habitación iluminada del laberinto. La llave desaparece en prima si Guillermo no la ha recogido.', 'The night action places key I on the altar and the spectacles in the illuminated maze room. The key disappears at prime if William has not collected it.'), action: t('Recoge la llave del altar durante esta noche. Permite acceder a la celda del Abad; procura que no te sorprenda al volver.', 'Collect the altar key during this night. It opens the Abbot’s cell; account for his watchfulness on your return.') },
